...
onMounted(() => {
  ...
  // 棋盘纹理
  let checkedBoardMaterial = new Cesium.CheckerboardMaterialProperty({
    evenColor: Cesium.Color.YELLOW, // 奇数行
    oddColor: Cesium.Color.PINK, // 偶数行
  ...
  let gridMaterial = new Cesium.GridMaterialProperty({
    ...
    lineThickness: new Cesium.Cartesian2(4.0, 4.0), // 指定沿每个轴的网格线的粗细
  });
  // 添加entity
  ...
  // viewer.camera.zoomIn(redPolygon);
  // 添加 primitive
  // 01-创建几何
  ...
  handler.setInputAction(function (movement) {
    ...
    if (Cesium.defined(pickedObject) && typeof pickedObject.id === "string") {
      ...
      let targetPrimitive = primitive.getGeometryInstanceAttributes(
        pickedObject.id,
      );
      targetPrimitive.color = Cesium.ColorGeometryInstanceAttribute.toValue(
        // Cesium.Color.YELLOW.withAlpha(0.5),
        // 可以设置随机颜色
        Cesium.Color.fromRandom({
          alpha: 0.5,
        }),
        targetPrimitive.color,
